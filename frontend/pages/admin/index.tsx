  ...
  return (
    <AdminLayout title="Overview">
      <div className="flex justify-between items-end mb-8">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight mb-1 text-gray-900 dark:text-white">Platform Analytics</h1>
          <p className="text-gray-500 dark:text-gray-400 text-sm">Welcome back! Here's the latest data from your drone fleets.</p>
        </div>
        <button className="px-5 py-2.5 bg-gray-900 dark:bg-white text-white dark:text-gray-900 text-sm font-bold rounded-xl shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all">
          + New Scan
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        {[
          { title: 'Active Projects', value: '24', icon: FolderKanban, color: 'from-blue-500 to-blue-600', trend: '+12% this month' },
          { title: 'Scanned Area', value: '1,250', unit: 'sq km', icon: Box, color: 'from-purple-500 to-purple-600', trend: '+450 sq km' },
          ...
          <div key={idx} className="bg-white dark:bg-[#111] border border-gray-200 dark:border-white/5 rounded-3xl p-6 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
            ...
            <h3 className="text-gray-500 dark:text-gray-400 text-sm font-medium mb-1 relative z-10">{kpi.title}</h3>
            <div className="flex items-baseline gap-1 mb-2 relative z-10 text-gray-900 dark:text-white">
      ...
      <div className="bg-white dark:bg-[#111] border border-gray-200 dark:border-white/5 rounded-3xl shadow-sm overflow-hidden">
        <div className="px-8 py-6 border-b border-gray-200 dark:border-white/5 flex justify-between items-center bg-gray-50/50 dark:bg-white/[0.02]">
          <h2 className="text-lg font-bold text-gray-900 dark:text-white">Recent Scans</h2>
          <button className="text-sm font-semibold text-blue-600 dark:text-cyan-400 hover:underline">View All</button>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-gray-200 dark:border-white/5 text-sm text-gray-500 dark:text-gray-400 bg-gray-50/50 dark:bg-transparent">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-white/5">
              {[
                ...
                { name: 'Bridge Pillar Deflection', loc: 'Mumbai', status: 'Processing', progress: 68, date: 'Today, 08:30 AM', icon: Clock, color: 'text-blue-500', bg: 'bg-blue-500' },
                { name: 'Highway Asphalt Volume', loc: 'Bangalore', status: 'Processing', progress: 34, date: 'Yesterday', icon: Clock, color: 'text-blue-500', bg: 'bg-blue-500' },
                ...
                <tr key={idx} className="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors group">
                  <td className="px-8 py-5">
                    <div className="font-bold text-gray-900 dark:text-white mb-1 group-hover:text-blue-600 dark:group-hover:text-cyan-400 transition-colors cursor-pointer">{row.name}</div>
                    <div className="text-xs text-gray-500">ID: PGI-2026-{9000 + idx}</div>
                  </td>
                  <td className="px-8 py-5 text-sm font-medium text-gray-600 dark:text-gray-300">{row.loc}</td>
                  <td className="px-8 py-5">
