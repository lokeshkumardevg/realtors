  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-stone-100 dark:bg-black text-gray-800 dark:text-white transition-colors duration-300 pt-24">
        ...
        {/* HERO SECTION */}
        <section className="relative py-20 lg:py-24 overflow-hidden bg-gradient-to-br from-gray-900 via-gray-800 to-black">
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[900px] h-[900px] bg-purple-500/40 blur-3xl rounded-full mix-blend-screen animate-[spin_60s_linear_infinite] z-0"></div>
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-pink-500/30 blur-3xl rounded-full mix-blend-screen animate-[spin_80s_linear_infinite_reverse] z-0"></div>
          ...
          <div className="relative z-20 container mx-auto px-4 text-center max-w-4xl">
            ...
            <h1 className="text-5xl md:text-6xl lg:text-7xl font-bold tracking-tight mb-6 text-white relative">
              ...
              <span className="absolute bottom-2 left-1/2 -translate-x-1/2 w-[250px] h-4 blur-xl bg-gradient-to-r from-gray-400/30 to-gray-500/30 z-0"></span>
              <br/>
                 ...
                 {categories.map(cat => {
                   ...
                   return (
                     <button 
                       ...
                       className={`px-6 py-2.5 rounded-full text-sm font-bold transition-all duration-300 ${isActive ? `bg-gradient-to-r ${grad} text-white shadow-[0_0_20px_rgba(168,85,247,0.4)]` : 'bg-white/5 text-gray-400 border border-white/10 hover:bg-white/10 hover:text-white'}`}
                     >
                       {cat}
                     </button>
                   );
          ...
          <div className="container mx-auto px-4 max-w-7xl">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
               {filteredVideos.map((vid, idx) => {
                 ...
                 return (
                   <div key={idx} onClick={() => setSelectedVideo(vid)} className="group relative rounded-3xl bg-white/5 border border-gray-200 dark:border-white/10 overflow-hidden cursor-pointer hover:-translate-y-2 hover:scale-[1.02] shadow-xl hover:shadow-2xl hover:shadow-purple-500/20 transition-all duration-500 flex flex-col">
                     ...
                     <div className="p-6 flex-1 flex flex-col">
                       <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2 group-hover:text-purple-400 transition-colors line-clamp-1">{vid.title}</h3>
                       <p className="text-gray-600 dark:text-gray-400 text-sm leading-relaxed line-clamp-2 flex-1">{vid.description}</p>
                     </div>
        ...
        {selectedVideo && (
          <div className="fixed inset-0 bg-black/95 z-[100] flex items-center justify-center p-4 md:p-6 transition-opacity">
            ...
            <div className="w-full max-w-6xl flex flex-col items-center">
               <video src={selectedVideo.url} controls autoPlay className="w-full rounded-2xl shadow-[0_0_50px_rgba(168,85,247,0.3)] bg-black aspect-video"></video>
